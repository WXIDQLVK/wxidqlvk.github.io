...
export default function CloudPlayer() {
  ...
  const [currentLyric, setCurrentLyric] = useState("正在加载歌词...");
  const [displayedLyric, setDisplayedLyric] = useState("");

  const audioRef = useRef<HTMLAudioElement>(null);
  const currentSong = localSongs[currentIndex];
  ...
  useEffect(() => {
    ...
    fetch(currentSong.lrc)
      .then(res => {
        if (!res.ok) throw new Error("本地歌词不存在");
        return res.text();
      })
      .then(text => {
        setLyrics(parseLrc(text));
        setCurrentLyric("♪ 本地歌词加载完毕 ♪");
      })
      .catch(() => {
        setCurrentLyric("♪ 本地歌词未找到 ♪");
      });
  ...
  const nextSong = () => setCurrentIndex(prev => (prev + 1) % localSongs.length);
  const prevSong = () => setCurrentIndex(prev => (prev - 1 + localSongs.length) % localSongs.length);
      ...
      <div className="md:col-span-5 rounded-3xl bg-white/40 dark:bg-slate-800/50 backdrop-blur-md border border-white/40 dark:border-white/10 shadow-xl p-6 flex flex-col justify-between transition-all duration-700 hover:scale-[1.02] relative group overflow-hidden min-h-[220px]">
        ...
        <div className="flex items-center gap-5 relative z-10 mb-6 mt-2">
          ...
          <div className="flex-col overflow-hidden w-full">
            <div className="flex items-center justify-between mb-1">
              <span className="text-[10px] font-black text-indigo-500 dark:text-indigo-400 tracking-widest uppercase bg-white/50 dark:bg-slate-900/50 px-2 py-0.5 rounded-sm shadow-sm">Local Music</span>
              <span className="text-xs font-bold text-slate-600 dark:text-slate-300 bg-white/40 dark:bg-slate-700/50 px-2 rounded-full">{currentIndex + 1} / {localSongs.length}</span>
            </div>
            <h3 className="text-xl font-bold text-slate-900 dark:text-white truncate drop-shadow-sm">{currentSong.title}</h3>
            <p className="text-sm text-slate-700 dark:text-slate-300 font-medium truncate drop-shadow-sm">{currentSong.artist}</p>
          </div>
        </div>
        ...
        <div className="relative z-10">
          ...
          <div className="flex items-center justify-center gap-6">
            <button onClick={prevSong} className="text-slate-700 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">
            ...
            <button onClick={nextSong} className="text-slate-700 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">
