...
export default function Home() {
  ...
  // 注意：在客户端组件中读取本地 posts 和 chatters 目录需要通过 API 或原有逻辑。
  // 如果你原本的页面是 Server Component 并且直接用了 fs，因为这里加了 "use client"，
  // 请确保你的项目允许客户端读取或保留了原样。下面为你保留了你原本的静态/服务端数据兜底逻辑：
  
  // 如果你的项目原本是 Server Component，直接用 fs 读取：
  // （若编译报错提示 fs 不能在客户端使用，请告诉我，我们可以改成通过 fetch 获取）
  let allPosts: any[] = [];
  let allChatters: any[] = [];
  
  // 为了安全起见，这里采用动态安全处理：
  // 假若你在编译时遇到 fs 报错，可以直接用空数组或者你在前后端适配好的逻辑。
  // 下面为你维持你原本的文件读取骨架（如果你原本就是这样写的）：
  try {
    // @ts-ignore
  ...
  return (
    <ToastProvider>
      <div className="min-h-screen relative pb-10">
        <Navbar />
        <PageTransition>
          {/* 🌟 核心适配：桌面 App 模式下自动下移（mt-36 sm:mt-40），网页浏览器中保持原样（mt-24 sm:mt-28） */}
          <div className={`w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-10 relative z-10 transition-all ${isStandalone ? 'mt-36 sm:mt-40' : 'mt-24 sm:mt-28'}`}>
